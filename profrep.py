...
def profile_to_csv(profile, OUTPUT, query_length, header, ax, cm):
...
def main(args):
	...
	for subfasta in fasta_list:
		...
		output_table = profile_to_csv(profile, OUTPUT, query_length, header, ax, cm)
	output_table.close()

	# Use gff module to convert profile table to GFF3 format
	if GFF:
		gff.main(OUTPUT, THRESHOLD)		
