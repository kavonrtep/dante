...
if args.new_db:
	subprocess.call("makeblastdb -in " + bl_database + " -dbtype nucl", shell=True)
...
## read fasta
def fasta_read(subfasta2):
	print(subfasta2)
	sequence_lines=[]
	with open(subfasta2, "r") as fasta:
		print(fasta)
...
def annot_profile(cl_annotations,query_len):
	...
	for key in list(cl_annotations.keys()):
		profile[key]=np.zeros(query_len,dtype=int)
	profile["repeat"] = np.zeros(query_len,dtype=int)
	profile["all"] = np.zeros(query_len,dtype=int)
	return profile 
...
def processInput(index):
	print('+++++++++++++++++++++++++++++')
	loc_start=index*step+1
	loc_end=index*step+window
	...
	for line in p.stdout:
		...
		if float(column[2]) >= identical & int(column[3]) >= align_length:	
			...
			if read in reads_annotations:
				...
				annotation="repeat"		
			#print(annotation)
			profile[annotation][qstart-step*index-1:qend-step*index-1]=profile[annotation][qstart-step*index-1:qend-step*index-1]+1
	return profile
...
def set_visualization(fasta_counter):
	fig=plt.figure(fasta_counter)
...
for subfasta in fasta_list:
	...
	## output dictionary
	#profile=annot_profile(cl_annotations,query_len)
		
	## set up the parallel process 
	step=window-overlap
	profile_parallel=[None]*17																	# moving step
	num_cores=multiprocessing.cpu_count()												# number of processors
	par=Pool(num_cores)																	# create pool 														
	subset=list(range(0,query_len,step))
	print(subset)
	index=list(range(len(subset)))												# define vector of indeces for parallel function
	print(index)
	...
	profile_list=par.map(processInput,index)			 										# create parallel process - output list of dictionaries for individual windows
	...
	## visualization
	[ax,cm]=set_visualization(fasta_counter)
	...
	########import function from GFF3 module#######
...
### csv # or just a list of the list of key value pairs
