...
def extract_nt_seqs(DNA_SEQ, DOM_GFF, OUT_DIR, CLASS_TBL, EXTENDED):
	...
	with open(DOM_GFF, "r") as domains:
		...
		for line in domains:
			...
			strand = line.split("\t")[6]
			align_nt_start = int(line.split("\t")[8].split(";")[3].split(":")[-1].split("-")[0])
			align_nt_end = int(line.split("\t")[8].split(";")[3].split(":")[-1].split("-")[1].split("[")[0])
			...
			if elem_type in unique_classes and "N" not in full_dom_nt:
				...
				if not elem_type in files_dict:
					files_dict[elem_type] = os.path.join(OUT_DIR, "{}.fasta".format(elem_type.split("|")[-1].replace("/","_")))
				with open(files_dict[elem_type], "a") as out_nt_seq:
					out_nt_seq.write(">{}:{}-{}|{}[{}]\n{}\n".format(seq_nt.id, dom_nt_start, dom_nt_end, dom_type, elem_type, textwrap.fill(full_dom_nt, configuration.FASTA_LINE)))
...
def write_domains_stat(domains_counts_dict, OUT_DIR):
	...
	'''
	total = 0
	with open(os.path.join(OUT_DIR, configuration.EXTRACT_DOM_STAT), "w") as dom_stat:
...
def main(args):
	...
	domains_counts_dict = extract_nt_seqs(DNA_SEQ, DOM_GFF, OUT_DIR, CLASS_TBL, EXTENDED)
	write_domains_stat(domains_counts_dict, OUT_DIR)
	
		
	print("ELAPSED_TIME_EXTRACTION = {} s\n".format(time.time() - t_nt_seqs_extraction))

if __name__ == "__main__":

    # Command line arguments
    parser = argparse.ArgumentParser()
    parser.add_argument('-i', '--input_dna', type=str, required=True,
						help='path to input DNA sequence')
    parser.add_argument('-d', '--domains_gff', type=str, required=True,
